...
// Add this map to store custom hover trigger positions for specific artwork IDs
const customHoverPositions = new Map<number, Vector3>();

// Add this function to set custom hover positions
export function setCustomHoverPosition(artworkId: number, offset: Vector3) {
    customHoverPositions.set(artworkId, offset);
}
...
export function createArtID(position: Vector3, rotation: Vector3, artworkId: number, artTitle: string, artDescription: string): Entity {
    ...
    // Get the hover position, using custom offset if defined
    const hoverOffset = customHoverPositions.get(artworkId) || Vector3.create(0, -2, 0);
    const hoverPosition = Vector3.create(
        position.x + hoverOffset.x,
        position.y + hoverOffset.y,
...
export function changeArtHoverSystem() {
    for (const [entity] of engine.getEntitiesWith(ArtHover, PointerEvents)) {
        ...
        if (inputSystem.isTriggered(InputAction.IA_POINTER, PointerEventType.PET_HOVER_ENTER, entity)) {
            hoverVisible = true;
            //Material.setPbrMaterial(entity, { albedoColor: Color4.White() }); // handy for debugging
...
export function createArtHovers() {
    // Set custom hover positions for artworks 25 and 26
    setCustomHoverPosition(25, Vector3.create(0, -5, 0)); // Move down 1.5m from default -2m
    setCustomHoverPosition(26, Vector3.create(0, -4.5, 0)); // Move down 1.5m from default -2m
    
    // Set custom hover positions for artworks 27, 30, and 31
    setCustomHoverPosition(27, Vector3.create(0, -2.5, 0)); // Move down 0.5m from default -2m
    setCustomHoverPosition(30, Vector3.create(0, -3, 0)); // Move down 0.5m from default -2m
    setCustomHoverPosition(31, Vector3.create(0, -2.5, 0)); // Move down 0.5m from default -2m
